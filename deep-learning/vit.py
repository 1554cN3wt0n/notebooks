...
def _():
    ...
    import requests
    return Image, np, os, requests, torch
...
def _(np, torch):
    def load_hparams_and_params(model_path):
        ...
        cls_token = (
            model["vit.embeddings.cls_token"].numpy().reshape(1, hidden_dim)
        )
        position_embeddings = (
        ...
        for i in range(n_encoder_layers):
            q = {
                ...
                "b": model[
                    f"vit.encoder.layer.{i}.attention.attention.query.bias"
                ].numpy(),
            }
            k = {
                "w": model[
                    f"vit.encoder.layer.{i}.attention.attention.key.weight"
                ].numpy(),
                "b": model[
                    f"vit.encoder.layer.{i}.attention.attention.key.bias"
                ].numpy(),
            }
            v = {
                "w": model[
                ...
                "b": model[
                    f"vit.encoder.layer.{i}.attention.attention.value.bias"
                ].numpy(),
            }
            c_attn = {
            ...
            mlp_c_fc = {
                "w": model[f"vit.encoder.layer.{i}.intermediate.dense.weight"]
                .numpy()
                .T,
                "b": model[
...
@app.cell
def _(convolution_2d, ffn, layer_norm, linear, mha, np, resize_bicubic):
    ...
    def vit(inputs, embeddings, encoder_blocks, ln_f, classifier, n_head):
        ...
        return logits[0]
    return transformer_block, vit, vit_embeddings, vit_interpolation
...
@app.cell
def _(Image, gauss_norm, np, params, requests, resize_image, vit):
    ...
    return image, logits, raw_img, url
